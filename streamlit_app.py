...
try:
    ...
    # 兼容处理换行符问题
    CRYPTOPANIC_API_KEY = st.secrets["CRYPTOPANIC_API_KEY"].strip().replace('\n', '').replace('"', '')
    KEYS_LOADED = True
except Exception:
    EXA_API_KEY = None
    GOOGLE_API_KEY = None
...
st.set_page_config(
    ...
    page_icon="🕵️‍♂️",
    layout="wide",
    initial_sidebar_state="collapsed" # 隐藏左侧原声边栏，我们要自己造右侧栏
)
...
<style>
    ...
    /* Crypto Ticker Item (Right Sidebar Style) */
    .crypto-item {
        display: flex;
    ...
    .c-name { color: #e2e8f0; font-weight: 700; font-size: 0.95rem; }
    .c-price { color: #f8fafc; font-weight: 400; font-size: 0.95rem; }
    
    /* 动态涨跌色块 */
    .c-change-pos { 
        color: #4ade80; font-size: 0.8rem; font-weight: 600;
        background: rgba(6, 78, 59, 0.5); padding: 4px 8px; border-radius: 6px; 
    }
    .c-change-neg { 
        color: #f87171; font-size: 0.8rem; font-weight: 600;
        background: rgba(127, 29, 29, 0.5); padding: 4px 8px; border-radius: 6px; 
    }
    ...
    /* Top 12 Grid */
    .top10-container { width: 100%; margin: 40px 0; }
    .top10-header {
        font-size: 0.9rem; color: #9ca3af; text-transform: uppercase;
    ...
    @media (max-width: 1200px) { .top10-grid { grid-template-columns: repeat(2, 1fr); } }
    ...
    .market-item {
        ...
        min-height: 110px; text-decoration: none !important; color: inherit !important;
    }
    .market-item:hover { border-color: #ef4444; background: rgba(31, 41, 55, 0.9); transform: translateY(-2px); }
    
    /* Tags */
    .tag-yes { background: rgba(6, 78, 59, 0.4); color: #4ade80; border: 1px solid rgba(34, 197, 94, 0.3); padding: 2px 8px; border-radius: 4px; font-weight: 600; }
    .tag-no { background: rgba(127, 29, 29, 0.4); color: #f87171; border: 1px solid rgba(239, 68, 68, 0.3); padding: 2px 8px; border-radius: 4px; font-weight: 600; }
</style>
...
# ================= 🖥️ 4. MAIN LAYOUT (WITH RIGHT SIDEBAR) =================

# 1. 创建左右布局：左边主控台 (3)，右边实时数据 (1)
col_main, col_right = st.columns([3, 1])

# --- 🟢 RIGHT COLUMN: LIVE CRYPTO FEED (AUTO-REFRESHING) ---
with col_right:
    st.markdown("<br><br><br>", unsafe_allow_html=True) # 调整顶部间距，对齐
    
    # 🔥 核心技术点：使用 st.fragment 让这部分独立刷新
    # run_every=3 表示每 3 秒刷新一次这个函数内部，页面其他部分不动！
    @st.fragment(run_every=3)
    def show_live_crypto():
        st.markdown("""<div style="color:#94a3b8; font-size:0.8rem; margin-bottom:10px; letter-spacing:1px; font-weight:600;">⚡ LIVE MARKET FEED</div>""", unsafe_allow_html=True)
        
        # 实时获取数据
        try:
            # 这里的 URL 我们用不需要 Key 的 Binance Public API
            url = "https://api.binance.com/api/v3/ticker/24hr"
            targets = ["BTCUSDT", "ETHUSDT", "SOLUSDT", "BNBUSDT", "DOGEUSDT", "XRPUSDT"]
            resp = requests.get(url, timeout=2).json()
            
            target_data = [x for x in resp if x['symbol'] in targets]
            
            for coin in target_data:
                ...
                # 涨跌样式
                change_class = "c-change-pos" if change >= 0 else "c-change-neg"
                ...
                <div class="crypto-item">
                    ...
                    <div style="text-align:right;">
                        <div class="c-price">${p_fmt}</div>
                        <span class="{change_class}">{change_sign}{change:.2f}%</span>
                    </div>
                </div>
                """, unsafe_allow_html=True)
...
with col_main:
    ...
    if top10_markets:
        ...
        st.markdown("""<div style="text-align:center; margin-top:50px; color:#666;">Connecting to Prediction Markets...</div>""", unsafe_allow_html=True)
    ...
    <style>
        ...
        .protocol-container { font-family: 'Inter', sans-serif; color: #cbd5e1; font-size: 0.95rem; line-height: 1.8; margin-top: 20px; text-align: center; display: flex; flex-direction: column; align-items: center; }
        .protocol-step { margin-bottom: 25px; padding: 15px 20px; border-radius: 12px; background: rgba(255, 255, 255, 0.03); border: 1px solid rgba(255, 255, 255, 0.05); max-width: 700px; width: 100%; transition: all 0.3s; }
        .protocol-step:hover { background: rgba(255, 255, 255, 0.05); border-color: rgba(255, 255, 255, 0.1); }
        .protocol-title { font-weight: 700; color: #ef4444; font-size: 1rem; letter-spacing: 0.5px; text-transform: uppercase; display: block; margin-bottom: 8px; }
        .credits-section { text-align: center; margin-top: 30px; padding-top: 20px; border-top: 1px solid #334155; color: #64748b; font-size: 0.85rem; font-family: monospace; }
    ...
    with st.expander("Operational Protocol & System Architecture"):
        ...
        if lang_mode == "EN":
            st.markdown("""<div class="protocol-container"><div class="protocol-step"><span class="protocol-title">1. Intelligence Injection (Input)</span>User inputs unstructured natural language data...</div><div class="protocol-step"><span class="protocol-title">2. Neural Semantic Mapping (Processing)</span>Powered by <b>Exa.ai</b>...</div><div class="protocol-step"><span class="protocol-title">3. Actionable Trading Insights (Analysis)</span><b>Google Gemini</b> utilizes neural semantic analysis...</div></div>""", unsafe_allow_html=True)
        else:
            st.markdown("""<div class="protocol-container"><div class="protocol-step"><span class="protocol-title">1. 情报注入 (Intelligence Injection)</span>用户输入非结构化自然语言数据...</div><div class="protocol-step"><span class="protocol-title">2. 神经语义映射 (Neural Mapping)</span>由 <b>Exa.ai</b> 驱动...</div><div class="protocol-step"><span class="protocol-title">3. 可执行交易洞察 (Actionable Insights)</span><b>Google Gemini</b> 利用神经语义分析...</div></div>""", unsafe_allow_html=True)
        st.markdown("""<div class="credits-section">SYSTEM ARCHITECTURE POWERED BY<br><span class="credits-highlight">Exa.ai</span> & <span class="credits-highlight">Google Gemini</span><br><br>Data Stream: Polymarket Gamma API</div>""", unsafe_allow_html=True)

# ================= ⚡ 底部实时滚动新闻条 (v2 开发者修复版) =================
@st.cache_data(ttl=300)
def fetch_ticker_news():
    try:
        api_key = st.secrets.get("CRYPTOPANIC_API_KEY", "").strip().replace('\n', '').replace('"', '')
        url = f"https://cryptopanic.com/api/developer/v2/posts/?auth_token={api_key}&public=true&filter=rising"
        resp = requests.get(url, timeout=10)
        if resp.status_code != 200: return [f"⚠️ Error {resp.status_code}", f"Msg: {resp.text[:50]}..."]
        data = resp.json()
        news_items = []
        if "results" in data:
            for item in data["results"][:15]:
                title = item["title"]
                code = "⚡"
                if "currencies" in item and item["currencies"]: code = f"[{item['currencies'][0]['code']}]"
...
st.markdown(f"""
<style>
    .news-ticker-container {{ position: fixed; bottom: 0; left: 0; width: 100%; background-color: #0f172a; border-top: 1px solid #dc2626; color: #e2e8f0; font-family: 'Inter', monospace; font-size: 0.9rem; padding: 8px 0; z-index: 9999; overflow: hidden; white-space: nowrap; box-shadow: 0 -4px 15px rgba(0,0,0,0.5); }}
    .ticker-wrap {{ width: 100%; overflow: hidden; }}
    .ticker-move {{ display: inline-block; white-space: nowrap; animation: ticker 120s linear infinite; }}
...
<div class="news-ticker-container"><div class="ticker-wrap"><div class="ticker-move">{ticker_text}</div></div></div><br><br><br>
""", unsafe_allow_html=True)
